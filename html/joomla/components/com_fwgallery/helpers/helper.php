...
if (!class_exists('JFHelper')) {
    ...
    class JFHelper {
		static function formatPrice($price) {
			...
			if ($params->get('currency_position') == 'after') return number_format($price, 2).$params->get('currency_label');
			else return $params->get('currency_label').number_format($price, 2);
		}
	...
	lft');
    	...
    	static function loadStylesheet() {
    		if (!defined('FWG_STYLESHEET_LOADED')) {
				...
				$params = JComponentHelper :: getParams('com_fwgallery');
				if ($params->get('use_own_bootstrap')) JHTML :: stylesheet('media/jui/css/bootstrap.min.css');
		...
		static function findGalleryImage($gallery, $prefix='') {
			...
			if (!is_array($galleries)) {
				...
				if (!$user->get('isRoot')) {
					if ($user->id) {
						$where[] = '(p.gid = 0 OR p.gid IS NULL OR EXISTS(SELECT * FROM #__usergroups AS ug WHERE pg.lft=ug.lft AND ug.id IN ('.implode(',',JAccess::getGroupsByUser($user->id, true)).')))';
