...
class Facebook
{
  ...
  protected function makeRequest($url, $params, $ch=null) {
	...
	$look_for_picture = preg_match('#/picture$#', $url);
	if ($look_for_picture) $opts[CURLOPT_HEADER] = true;
    ...
    if ($look_for_picture and preg_match('#^Location: (.*)$#m', $result, $match)) {
    	/* emulate receiveing of JSON data */
    	$result = '{"link":"'.addcslashes(trim($match[1]), '"/').'"}';
    }
    return $result;
  }
