...
class ComDocmanViewHtml extends ComKoowaViewHtml
{
    ...
    protected function _generatePathway($category = null, $document = null)
    {
        ...
        if (!in_array($view, array('document', 'list', 'userlist', 'submit'))
            || ($query['view'] === 'document' && $view === 'document' && $query['slug'] === $document->slug)
            || (in_array($query['view'], array('list', 'userlist')) && $view === $query['view']
                    && !empty($category) && $query_slug === (string) $category->slug)
        ) {
            return;
        }

        if (!in_array($query['view'], array('filteredlist')) && $category && $query_slug !== $category->slug)
        {
            $vars = array(
                'option' => 'com_docman',
                'view'   => $query['view'],
