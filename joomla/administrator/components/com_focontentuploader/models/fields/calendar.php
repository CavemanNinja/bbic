...
class JFormFieldCalendar extends JFormField
{
	...
	protected function getInput()
	{
        ...
        if ((int)$this->value) {
            ...
            $inputvalue = strftime($format, strtotime($this->value));
        ...
        $document->addScriptDeclaration(
            'jQuery(document).ready(function($) {
                $("#' . $this->id . '").datePicker().dpSetOffset(28, -135);
            });');

        $attribs = JArrayHelper::toString($attributes);
        $output = '<div class="input-append">'
            . '<input type="text" title=""'
            . ' name="' . $this->name . '" id="' . $this->id . '" value="' . htmlspecialchars($this->value, ENT_COMPAT, 'UTF-8') . '" ' . $attribs . ' />'
            . '<button type="button" class="btn" id="' . $this->id . '_img" tabindex="9999"><i class="icon-calendar"></i></button>'
        . '</div>';
